  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[#19315d]/50 bg-[#07111f]/60 p-5 space-y-4">
        <h4 className="text-sm font-semibold text-slate-300">Package Duration</h4>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-slate-400">Days</label>
            <input type="number" className={`mt-2 ${inp}`} placeholder="8" value={days}
              onChange={(e) => onChange('days', e.target.value)} />
          </div>
          <div>
            <label className="text-sm text-slate-400">Nights</label>
            <input type="number" className={`mt-2 ${inp}`} placeholder="7" value={nights}
      ...
      {/* Breakdown */}
      <div className="rounded-xl border border-[#19315d]/50 bg-[#07111f]/60 p-5 space-y-4">
        <h4 className="text-sm font-semibold text-slate-300">Duration Breakdown</h4>

        <div className="space-y-3">
          {breakdown.map((item, idx) => (
            <div key={item.id} className="flex items-center gap-3 rounded-xl border border-[#19315d]/40 bg-[#0b1730]/60 p-3">
              <span className="text-xs text-slate-600 w-5 shrink-0">{idx + 1}</span>
              <input type="number" className={`${inp} w-24 shrink-0`} placeholder="Days" value={item.days}
                onChange={(e) => setBreakdown(breakdown.map((i) => i.id === item.id ? { ...i, days: e.target.value } : i))} />
              <input className={`${inp} flex-1`} placeholder="Place (e.g. Gulmarg)" value={item.place}
                onChange={(e) => setBreakdown(breakdown.map((i) => i.id === item.id ? { ...i, place: e.target.value } : i))} />
              <button type="button"
                onClick={() => setBreakdown(breakdown.filter((i) => i.id !== item.id))}
        ...
        <button type="button"
          onClick={() => setBreakdown([...breakdown, { id: uid(), days: '1', place: '' }])}
