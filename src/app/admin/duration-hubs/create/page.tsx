...
import {
  ...
  Users, ImageIcon, Images, Clock, MessageSquareText, BarChart3,
} from 'lucide-react';
...
const STORAGE_KEY = 'cms_durationhub_create_draft';
...
// Builds the URL slug from day/night counts: 6, 5 → "6-days-5-nights"
const buildSlug = (days: string, nights: string) =>
  days && nights ? `${days}-days-${nights}-nights` : '';

export default function CreateDurationHubPage() {
  const router = useRouter();

  const [form, setForm] = useState<DurationHubForm>(INITIAL);
  ...
  const f = (field: keyof DurationHubForm, value: string | boolean) => {
    setForm((prev) => {
      ...
      // Auto-generate slug whenever days or nights change
  ...
  const post = async (status: 'draft' | 'published') => {
    const res = await fetch('/api/duration-hubs', {
      method: 'POST',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast('Duration hub published!', 'success');
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <CMSSection title="Quick Answer" icon={<MessageSquareText className="w-4 h-4" />} defaultOpen>
          <div className="space-y-2 mt-1">
            ...
            <textarea
              className={ta}
              placeholder="A concise, direct answer shown near the top — powers featured snippets & AI answers. e.g. A 6 days 5 nights Kashmir tour covers Srinagar, Gulmarg and Pahalgam with..."
