...
import {
  ...
  Eye, EyeOff, Landmark, SortDesc, SortAsc, Tag, Filter,
  X, MapPin, AlertTriangle, CalendarDays,
} from 'lucide-react';
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-semibold text-base">Delete Temple?</h3>
                <p className="text-slate-500 text-xs">This action cannot be undone</p>
              </div>
            </div>
            <p className="text-slate-400 text-sm mb-6 pl-1">
              The temple listing will be permanently removed from the database.
            </p>
...
// ─── Temple Cards ───────────────────────────────────────────
function TempleCards({ temples, onDelete }: { temples: TempleItem[]; onDelete: (id: string) => void }) {
  return (
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1.5 mb-4 text-[11px] text-slate-500">
              {temple.templeType && (
                <span className="flex items-center gap-1">
                  <Tag className="w-3 h-3 shrink-0" /> {temple.templeType}
                </span>
              )}
              {temple.location && (
                <span className="flex items-center gap-1">
...
// ─── Temple Table ───────────────────────────────────────────
function TempleTable({ temples, onDelete }: { temples: TempleItem[]; onDelete: (id: string) => void }) {
  return (
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#19315d]/40">
              <th className="text-left px-5 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Temple</th>
              <th className="text-left px-4 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-widest hidden md:table-cell">Type</th>
              <th className="text-left px-4 py-3.5 text-[11px] font-bold text-slate-500 uppercase tracking-widest hidden lg:table-cell">Location</th>
              ...
              >
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-white text-sm font-medium truncate max-w-52">{temple.title}</p>
                      <p className="text-slate-600 text-xs truncate max-w-52">/temples/{temple.slug}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-3.5 text-center">
                  <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full border ${
                    ...
                    {temple.status === 'published' ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
                    {temple.status}
                  </span>
                </td>
                <td className="px-4 py-3.5 hidden lg:table-cell">
                  <span className="flex items-center gap-1.5 text-xs text-slate-500">
                    ...
                    {temple.createdAt
                      ? new Date(temple.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                      : '—'}
                  </span>
                </td>
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-2 justify-end">
                    <Link href={`/admin/temples/${temple._id}/edit`}
                      className="p-2 rounded-lg text-slate-500 hover:text-blue-400 hover:bg-blue-600/10 transition-all">
                      <Pencil className="w-3.5 h-3.5" />
                    </Link>
                    <button onClick={() => onDelete(temple._id)}
...
export default function TemplesPage() {
  ...
  const fetchTemples = async () => {
    try {
      ...
      showToast('Failed to load temples', 'err');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/temples/${selectedId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      setTemples((prev) => prev.filter((t) => t._id !== selectedId));
      showToast('Temple deleted successfully', 'ok');
      setModalOpen(false);
    } catch (err) {
  ...
  const filtered = temples
    .filter((t) => {
      ...
      const matchSearch =
        ...
        (t.templeType ?? '').toLowerCase().includes(q) ||
  ...
  const drafts = temples.filter((t) => t.status === 'draft').length;
  ...
  return (
    <section className="min-h-screen pb-16">
      ...
      {/* ── Header ── */}
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-blue-50">Temple Management</h1>
        <p className="text-sm text-blue-300/50 mt-0.5">Manage Kashmir&apos;s temples & shrines</p>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-28 gap-4">
          ...
          <p className="text-slate-600 text-sm">Loading temples...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-28 gap-3 text-slate-600">
          <Landmark className="w-10 h-10" />
          <p className="text-sm font-medium">No temples found</p>
          {temples.length === 0 ? (
            <Link href="/admin/temples/create" className="mt-1 text-blue-400 text-sm hover:underline">
        ...
        <TempleCards temples={filtered} onDelete={openDelete} />
      ) : (
        <TempleTable temples={filtered} onDelete={openDelete} />
      )}
    </section>
  );
}
