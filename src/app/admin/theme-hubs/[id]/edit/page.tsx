...
import {
  ...
  Users, ImageIcon, Images, Tag, MessageSquareText, BarChart3, Loader2,
} from 'lucide-react';
...
export default function EditThemeHubPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();

  const [form, setForm] = useState<ThemeHubForm>(EMPTY);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`/api/theme-hubs/${id}`);
        if (!res.ok) throw new Error('Not found');
        const { themeHub: c } = await res.json();
        ...
        showToast('Failed to load theme hub', 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast('Theme hub updated!', 'success');
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-4">
        ...
        <p className="text-slate-600 text-sm">Loading theme hub...</p>
      </div>
    );
  }
