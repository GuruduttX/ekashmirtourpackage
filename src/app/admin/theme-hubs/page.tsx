...
import {
  ...
  Star, Tag, AlertTriangle, X, Filter,
} from 'lucide-react';
import CountUp from '@/utils/CountUp';

// ─── Types ──────────────────────────────────────────────
interface ThemeHubItem {
  _id: string;
  slug: string;
  themeName: string;
  title: string;
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-semibold text-base">Delete Theme Hub?</h3>
                <p className="text-slate-500 text-xs">This action cannot be undone</p>
              </div>
            </div>
            <p className="text-slate-400 text-sm mb-6 pl-1">
              The theme hub page will be permanently removed from the database.
            </p>
...
// ─── Cards ───────────────────────────────────────────────
function HubCards({ hubs, onDelete }: { hubs: ThemeHubItem[]; onDelete: (id: string) => void }) {
  return (
        ...
        >
          ...
          <div className="p-4">
            ...
            <div className="flex gap-2">
              <Link
                href={`/admin/theme-hubs/${hub._id}/edit`}
...
// ─── Main Page ──────────────────────────────────────────
export default function ThemeHubsPage() {
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');

  const [hubs, setHubs] = useState<ThemeHubItem[]>([]);
  ...
  const fetchHubs = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/theme-hubs', { cache: 'no-store' });
      const data = await res.json();
      setHubs(data.themeHubs ?? []);
    } catch {
      showToast('Failed to load theme hubs', 'err');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/theme-hubs/${selectedId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      setHubs((prev) => prev.filter((h) => h._id !== selectedId));
      showToast('Theme hub deleted successfully', 'ok');
      setModalOpen(false);
    } catch (err) {
  ...
  const filtered = hubs.filter((h) => {
    ...
    const matchSearch =
      ...
      h.themeName.toLowerCase().includes(q) ||
  ...
  return (
    <section className="min-h-screen pb-16">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-blue-50">Theme Hub Management</h1>
        <p className="text-sm text-blue-300/50 mt-0.5">Theme-based landing pages</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <div className="relative flex-1 min-w-52">
          ...
          <input
            placeholder="Search by title, theme, slug..."
            value={search}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-28 gap-4">
          ...
          <p className="text-slate-600 text-sm">Loading theme hubs...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-28 gap-3 text-slate-600">
          <Tag className="w-10 h-10" />
          <p className="text-sm font-medium">No theme hubs found</p>
          {hubs.length === 0 ? (
            <Link href="/admin/theme-hubs/create" className="mt-1 text-blue-400 text-sm hover:underline">
              Create your first theme hub →
            </Link>
          ) : (
