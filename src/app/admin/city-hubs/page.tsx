...
import {
  Search, Plus, Pencil, Trash2, Eye, EyeOff, MapPin,
  Star, Navigation, AlertTriangle, CalendarDays, X, Filter,
} from 'lucide-react';
import CountUp from '@/utils/CountUp';

// ─── Types ──────────────────────────────────────────────
interface CityHubItem {
  _id: string;
  slug: string;
  cityName: string;
  title: string;
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-semibold text-base">Delete City Hub?</h3>
                <p className="text-slate-500 text-xs">This action cannot be undone</p>
              </div>
            </div>
            <p className="text-slate-400 text-sm mb-6 pl-1">
              The city hub page will be permanently removed from the database.
            </p>
...
// ─── Cards ───────────────────────────────────────────────
function HubCards({ hubs, onDelete }: { hubs: CityHubItem[]; onDelete: (id: string) => void }) {
  return (
        ...
        >
          ...
          <div className="p-4">
            ...
            <div className="flex gap-2">
              <Link
                href={`/admin/city-hubs/${hub._id}/edit`}
...
export default function CityHubsPage() {
  ...
  const [hubs, setHubs] = useState<CityHubItem[]>([]);
  ...
  const fetchHubs = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/city-hubs', { cache: 'no-store' });
      const data = await res.json();
      setHubs(data.cityHubs ?? []);
    } catch {
      showToast('Failed to load city hubs', 'err');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/city-hubs/${selectedId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      setHubs((prev) => prev.filter((h) => h._id !== selectedId));
      showToast('City hub deleted successfully', 'ok');
      setModalOpen(false);
    } catch (err) {
  ...
  return (
    <section className="min-h-screen pb-16">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-blue-50">City Hub Management</h1>
        <p className="text-sm text-blue-300/50 mt-0.5">Origin-city landing pages</p>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-28 gap-4">
          ...
          <p className="text-slate-600 text-sm">Loading city hubs...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-28 gap-3 text-slate-600">
          <Navigation className="w-10 h-10" />
          <p className="text-sm font-medium">No city hubs found</p>
          {hubs.length === 0 ? (
            <Link href="/admin/city-hubs/create" className="mt-1 text-blue-400 text-sm hover:underline">
              Create your first city hub →
            </Link>
          ) : (
