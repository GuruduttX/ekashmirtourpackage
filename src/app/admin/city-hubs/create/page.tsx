...
import {
  ...
  Users, ImageIcon, Images, Navigation, MessageSquareText, BarChart3,
} from 'lucide-react';
...
interface CityHubForm {
  title: string; cityName: string; slug: string;
...
const STORAGE_KEY = 'cms_cityhub_create_draft';
...
export default function CreateCityHubPage() {
  const router = useRouter();

  const [form, setForm] = useState<CityHubForm>(INITIAL);
  ...
  const f = (field: keyof CityHubForm, value: string | boolean) => {
    setForm((prev) => {
      const next = { ...prev, [field]: value };
      // Auto-generate slug from city name: "Delhi" → "from-delhi"
      if (field === 'cityName') {
  ...
  const buildPayload = (status: 'draft' | 'published') => ({
    title: form.title, cityName: form.cityName, slug: form.slug,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast('City hub published!', 'success');
      setTimeout(() => router.push('/admin/city-hubs'), 1200);
    } catch (err) {
  ...
  return (
    <div className="w-full py-8 pb-24">
      <CMSHeader editorType="City Hub" />

      <form onSubmit={handleSubmit} className="space-y-6">
        {/* City Hub Details */}
        <CMSSection title="City Hub Details" icon={<Navigation className="w-4 h-4" />} defaultOpen>
          <div className="space-y-5 mt-1">
            <div>
              ...
              <input
                className={`mt-2 ${inp}`}
                placeholder="e.g. Kashmir Tour Packages from Delhi"
                value={form.title}
                required
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input
                  ...
                  value={form.cityName}
                  required
                  onChange={(e) => f('cityName', e.target.value)}
                />
                <p className="text-xs text-slate-500 mt-1.5">
                  Must match the package Source City value (used to filter packages).
                </p>
              </div>
              <div>
                <label className="text-sm text-slate-400">
                ...
                />
                <p className="text-xs text-slate-500 mt-1.5">
                  /kashmir-tour-packages/{form.slug || 'from-delhi'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 mt-1">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  className={`mt-2 ${inp}`}
                  placeholder="e.g. from Delhi (highlighted phrase)"
              ...
              <div>
                ...
                <input
                  className={`mt-2 ${inp}`}
                  placeholder="e.g. Handpicked journeys, doorstep pickup from Delhi"
                  value={form.subtitle}
        ...
        <CMSSection title="Quick Answer" icon={<MessageSquareText className="w-4 h-4" />} defaultOpen>
          <div className="space-y-2 mt-1">
            ...
            <textarea
              className={ta}
              placeholder="A concise, direct answer shown near the top — powers featured snippets & AI answers. e.g. Yes, we offer all-inclusive Kashmir tour packages from Delhi with..."
        ...
        <CMSSection title="Overview (optional)" icon={<AlignLeft className="w-4 h-4" />} defaultOpen={false}>
          <div className="mt-1">
            ...
            <p className="text-xs text-slate-500 mt-2">
              Optional unique copy for this city (flights/trains, transit tips). Leave blank to hide it on the page.
            </p>
          </div>
        </CMSSection>
        ...
        <CMSSection title="Gallery" icon={<Images className="w-4 h-4" />} defaultOpen={false} badge={gallery.length || undefined}>
          <div className="mt-1">
            <GalleryHandler images={gallery} setImages={setGallery} editorType="CityHub" />
          </div>
        </CMSSection>
        ...
        <CMSSection title="SEO">
          <div className="mt-1">
            <CMSSeoSection
              ...
              onChange={(field, val) => f(field as keyof CityHubForm, val)}
            />
          </div>
        </CMSSection>
