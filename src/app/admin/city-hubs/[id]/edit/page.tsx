...
import {
  ...
  Users, ImageIcon, Images, Navigation, MessageSquareText, BarChart3, Loader2,
} from 'lucide-react';
...
export default function EditCityHubPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();

  const [form, setForm] = useState<CityHubForm>(EMPTY);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`/api/city-hubs/${id}`);
        if (!res.ok) throw new Error('Not found');
        const { cityHub: c } = await res.json();
        ...
        showToast('Failed to load city hub', 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast('City hub updated!', 'success');
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-4">
        ...
        <p className="text-slate-600 text-sm">Loading city hub...</p>
      </div>
    );
  }
